...
This repository is currently in the planning stage for **Agent Ready**, an open source Node.js CLI that checks whether a project is ready for coding agents.
...
- Generated local reports should use `.agent-ready/` and should not be committed unless intentionally documented as fixtures.
...
Use `camelCase` for variables and functions, `PascalCase` for types/classes, and kebab-case for CLI-facing names such as `agent-ready`. Follow Prettier once configured. Avoid dependencies unless they solve a real problem.
...
Do not commit secrets, `.env` files, tokens, or private keys. Agent Ready should not require API keys or hosted services for the MVP. Discuss changes that affect architecture, distribution, security, or public CLI behavior before implementation.
